...
class ModerationHandlers:
    ...
    async def _handle_unban(self, query, context, chat_id: int, target_id: int, admin_name: str) -> None:
        ...
        if success:
            await query.edit_message_text(Messages.unban_notification(user, admin_name), parse_mode="Markdown")
        else:
    ...
    async def trust_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(Messages.whitelist_added(user), parse_mode="Markdown")
    ...
    async def untrust_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not update.message.reply_to_message:
            await update.message.reply_text("ℹ️ Ответь на сообщение пользователя командой /untrust")
            return
        ...
        if removed:
            user = UserInfo(target.id, target.first_name, target.username)
            await update.message.reply_text(Messages.whitelist_removed(user), parse_mode="Markdown")
        else:
