...
class SteamLinkRepository:
    ...
    async def get_account_id(self, user_id: int) -> Optional[int]:
        ...
        async with self.db.connection() as conn:
            cursor = await conn.execute(
                "SELECT account_id FROM steam_links WHERE user_id = ?",
                (user_id,)
            )
    ...
    async def get_all_linked(self) -> List[Tuple[int, int, str]]:
        ...
        async with self.db.connection() as conn:
            cursor = await conn.execute(
                "SELECT user_id, account_id, persona_name FROM steam_links"
            )
    ...
    async def unsubscribe_shame(self, user_id: int, chat_id: int) -> bool:
        ...
        async with self.db.connection() as conn:
            cursor = await conn.execute(
                "DELETE FROM shame_subscriptions WHERE user_id = ? AND chat_id = ?",
                (user_id, chat_id)
            )
            return cursor.rowcount > 0
    ...
    async def is_shame_subscribed(self, user_id: int, chat_id: int) -> bool:
        ...
        async with self.db.connection() as conn:
            cursor = await conn.execute(
                "SELECT 1 FROM shame_subscriptions WHERE user_id = ? AND chat_id = ?",
                (user_id, chat_id)
            )
    ...
    async def get_all_shame_chats(self) -> List[int]:
        ...
        async with self.db.connection() as conn:
            cursor = await conn.execute(
                "SELECT DISTINCT chat_id FROM shame_subscriptions"
            )
            results = await cursor.fetchall()
