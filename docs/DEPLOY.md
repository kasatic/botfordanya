...
Все зависимости устанавливаются автоматически из `requirements.txt`.
...
После деплоя Dota 2 интеграция работает автоматически:

### Доступные команды
| Команда | Описание |
|---------|----------|
| `/link <id>` | Привязать Steam (Friend ID, Steam ID 64, ссылки) |
| `/unlink` | Отвязать Steam аккаунт |
| `/game` | Проверить в игре ли (ответом на сообщение — другого) |
| `/lastgame` | Краткая инфа о последнем матче |
| `/last` | Детальная аналитика (GPM/XPM, урон, ранги в команде) |
| `/profile` | Профиль с рангом |
| `/toxic` | Анализ токсичности (wordcloud из чата Dota) |
| `/shame on/off` | Подписка на "позор" после матчей |

### Shame-сервис
- Работает в фоне, проверяет матчи каждые 2 минуты
- Находит самого бесполезного игрока среди подписчиков
- Автоматически постит "позор" в чат

### OpenDota API
Бот использует бесплатный [OpenDota API](https://docs.opendota.com/) без ключа.
Лимиты: ~60 запросов/минуту (достаточно для небольших чатов).
