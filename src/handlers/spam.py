...
class SpamHandlers:
    ...
    async def _process_spam(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE, spam_type: SpamType, content_hash: str = None
    ) -> None:
        ...
        user = UserInfo(user_id=user_id, name=update.effective_user.first_name, username=update.effective_user.username)
        ...
        success, violation_count, ban_minutes = await self.ban_service.apply_ban(
            context, chat_id, user_id, ban_type=spam_type.value, reason=f"{result.count} {result.reason}"
        )

        if success:
            text = Messages.ban_notification(
                user, violation_count, ban_minutes, result.count, result.reason, spam_type.value
            )
        else:
            text = (
        ...
        await context.bot.send_message(
            chat_id, text, parse_mode="Markdown", reply_markup=Keyboards.ban_actions(user_id)
        )
